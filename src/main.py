...
def main(page: ft.Page):
    try:
            ...
            if e.files and len(e.files) > 0:
                ...
                try:
                    with open(img_path, 'rb') as f:
                        response = requests.post('https://api.qrserver.com/v1/read-qr-code/', files={'file': f}, timeout=15)
        ...
        btn_scan = ft.Container(
            content=ft.Row([ft.Icon(ft.Icons.CAMERA_ALT, size=24, color=ft.Colors.WHITE), ft.Text("التقاط صورة للبطاقة", size=18, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE)], alignment=ft.MainAxisAlignment.CENTER),
        ...
        view_scan = ft.Container(
            content=create_glass_card(
                ft.Column(
                    ...
                    alignment=ft.MainAxisAlignment.CENTER, # للتوسيط العمودي
                )
            ),
            padding=20, 
            alignment=ft.Alignment(0, 0),
            expand=True # أمر حاسم للهواتف
        )
        ...
        def refresh_attendance_list():
            ...
            if not db_attendance:
                attendance_list_view.controls.append(ft.Text("لا يوجد حضور حتى الآن.", text_align=ft.TextAlign.CENTER, color=ft.Colors.WHITE54))
        ...
        view_dashboard = ft.Container(
            ...
            ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, expand=True), # توسيع العمود
            padding=20,
            expand=True # أمر حاسم للهواتف
        )
        ...
        def load_csv_data(file_path):
            try:
                ...
                show_snack_bar(f"خطأ في قراءة الملف", ft.Colors.RED_400)

        path_input = ft.TextField(label="مسار ملف الطلاب", hint_text="ألصق مسار الـ CSV هنا...", border_color=ft.Colors.PURPLE_400, expand=True) # تمدد تلقائي للحقل
        btn_sync = ft.IconButton(icon=ft.Icons.SYNC, icon_color=ft.Colors.WHITE, bgcolor=ft.Colors.BLUE_700, on_click=lambda _: load_csv_data(path_input.value.strip()) if path_input.value else show_snack_bar("أدخل المسار أولاً", ft.Colors.RED_400))
        row_file_selector = ft.Row(controls=[path_input, btn_sync], alignment=ft.MainAxisAlignment.CENTER, spacing=10)
        ...
        btn_generate = ft.Container(content=ft.Text("توليد بطاقة QR", weight=ft.FontWeight.BOLD), bgcolor=ft.Colors.BLUE_700, padding=10, border_radius=8, width=150, alignment=ft.Alignment(0, 0), on_click=on_generate_click, ink=True)

        view_admin = ft.Container(
            content=ft.ListView( # استخدام ListView بدلاً من Column لتفادي مشاكل الشاشات الصغيرة
                controls=[
                    create_glass_card(ft.Column([ft.Text("إدارة قاعدة البيانات", size=18, weight=ft.FontWeight.BOLD), row_file_selector, stats_text], horizontal_alignment=ft.CrossAxisAlignment.CENTER)),
                    ft.Container(height=20),
                    create_glass_card(ft.Column([ft.Text("إصدار بطاقات QR", size=18, weight=ft.FontWeight.BOLD), gen_id_input, btn_generate, qr_image], horizontal_alignment=ft.CrossAxisAlignment.CENTER)),
                ],
                expand=True # أمر حاسم
            ),
            padding=20,
            expand=True # أمر حاسم للهواتف
        )
        ...
        # ----------------------------------------
        # شريط التنقل السفلي والترتيب النهائي
        # ----------------------------------------
        views = [view_scan, view_dashboard, view_admin]
        main_content = ft.Container(content=views[0], expand=True) # تمدد المحتوى الأساسي
        ...
        # SafeArea تحمي التطبيق من التداخل مع شريط الحالة العلوي (الساعة والبطارية) في الأندرويد
        page.add(ft.SafeArea(main_content, expand=True))

    # مصيدة الأخطاء
    except Exception as e:
        error_msg = f"حدث خطأ قاتل أثناء التحميل:\n{str(e)}\n\n{traceback.format_exc()}"
        page.add(ft.SafeArea(ft.ListView(controls=[ft.Text(error_msg, color="red", size=16, selectable=True)], expand=True), expand=True))
